...
def lookup_supersedence(kb):
    try:
        ...
        kbids = set()
        p = Progress(
            name = "\t- Opzoeken missende KB%s " % (kb),
            width = len(ids),
        )
        for uid in ids:
        ...
        if len(kbids) == 0:
            p.finish(msg=colored("Geen achterhaalde KB's gevonden", "cyan"))
        else:
            p.finish(msg=colored("\n\t\tGevonden: ", "green") + "%s" % (found_kbids))

        return [kbid.lstrip("KB") for kbid in kbids]
    except KeyboardInterrupt:
        print(colored("\nKeyboardInterrupt opgevangen.  Afsluiten...", "cyan"))
        exit()
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Voer dit script uit met een Microsoft HotfixID als het enige argument,")
        print("Voorbeeld: python muc_lookup.py KB4515384")
        sys.exit(1)

    kb = re.sub('KB', '', sys.argv[1], flags=re.IGNORECASE)
    kbids = lookup_supersedence(kb)
    print("{} is vervangen door: {}".format(kb, ["KB{}".format(kbid) for kbid in kbids]))
