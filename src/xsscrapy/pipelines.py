...
class XSSCharFinder(object):
    ...
    def get_filename(self, url):
        ...
        date_now = str(date_today) + '--' + str(current_time)
        # date = str(date.today()) + "--" + str(current_time)
    ...
    def process_item(self, item, spider):
        ...
        body = body.lower().decode("utf-8", "ignore")
    ...
    def sqli_check(self, body, orig_body):
        ...
        for (dbms, regex) in ((dbms, regex) for dbms in DBMS_ERRORS for regex in DBMS_ERRORS[dbms]):
            if re.search(regex, body.decode("utf-8", "ignore"), re.I) and not re.search(regex, orig_body.decode("utf-8", "ignore"), re.I):
                return (dbms, regex)
        return None, None
    ...
    def unescape_payload(self, payload):
        ...
        if '%' in payload:
            payload = urllib3.parse.unquote_plus(payload)
    ...
    def write_to_file(self, item, spider):
        with open(self.filename, 'a+') as f:
            ...
            f.write('[i] URL: '+item['orig_url']+'\n')
            spider.log('    URL: '+item['orig_url'], level=INFO)

            f.write('[i] Response URL: '+item['resp_url']+'\n')
            spider.log('    Response URL: '+item['resp_url'], level=INFO)

            if 'POST_to' in item:
                f.write('[i] POST URL: '+item['POST_to']+'\n')
                spider.log('    POST URL: '+item['POST_to'], level=INFO)

            f.write('[+] Ongefilterde tekens: '+item['unfiltered']+'\n')
            spider.log('    Ongefilterde tekens: '+item['unfiltered'], level=INFO)
            ...
            f.write('[+] Injectiepunten: '+item['xss_param']+'\n')
            spider.log('    Injectiepunten: '+item['xss_param'], level=INFO)

            if 'sugg_payloads' in item:
                f.write('[+] Mogelijke payloads: '+item['sugg_payloads']+'\n')
                spider.log('    Mogelijke payloads: '+item['sugg_payloads'], level=INFO)

            # Cut off the line at 500
            #f.write('Line: '+item['line'][-500:]+'\n')
            f.write('[i] Line: '+item['line']+'\n')
            spider.log('    Line: '+item['line'], level=INFO)

            if 'error' in item:
                f.write('[!] Fout: '+item['error']+'\n')
                spider.log('    Fout: '+item['error'], level=INFO)
