...
def main():
    ...
    if hasattr(args, 'perform_update') and args.perform_update:
        print(colored('[+] Definities updaten...', 'green'))
        ...
        print(colored('[+] Ontvangen definities gecreeërd op ', 'green') + '%s' % colored(date, 'yellow'))
        return

    # Update application
    if hasattr(args, 'perform_wesupdate') and args.perform_wesupdate:
        print(colored('[+] Wes.py updaten...', 'green'))
        urlretrieve('https://raw.githubusercontent.com/bitsadmin/wesng/master/wes.py', 'wes.py')
        print(colored('[+] Geüpdatet naar de laatste versie van wes.py. Start de tool opnieuw op om deze te gebruiken.', 'green'))
        return
    ...
    if hasattr(args, 'showversion') and args.showversion:
        cves, date = load_definitions('definitions.zip')
        print('Wes.py versie: %.2f' % VERSION)
        print('Database versie: %s' % date)
        return

    # Using the list of missing patches as a base
    if hasattr(args, 'missingpatches') and args.missingpatches:
        print(colored('[+] Definities laden', 'green'))
        cves, date = load_definitions('definitions.zip')

        # Obtain IDs of missing patches from file
        print(colored('[+] IDs van missende patches laden uit file ', 'green'))
        missingpatches = []
        ...
        if os_name:
            print(colored('[+] Kwetsbaarheden filteren voor "%s"' % os_name, 'green'))
        ...
        if os_name and 'Windows Server' in os_name:
            print(colored('[+] Uitfilteren van dubbele kwetsbaarheden ', 'green'))
    ...
    # Using systeminfo.txt or qfe.txt with list of installed patches as a base
        ...
        if hasattr(args, 'qfefile') and args.qfefile:
            ...
            if (hasattr(args, 'operating_system') and args.operating_system and args.operating_system.isdigit()) or \
                ...
                print('    - Aanmaakdatum van definities: %s' % date)
            ...
            if not args.operating_system:
                ...
                # Quit script
                print(colored('[i] Run het script opnieuw met de --os parameter en geef de index of naam van de OS waar u naar wilt zoeken.', 'yellow'))
                exit(0)
            else:
        ...
        if hasattr(args, 'systeminfo') and args.systeminfo:
            info += ('\n'
                     ...
                     '    - Bouw: %s\n'
                     '    - Versie: %s\n'
                     '    - Architectuur: %s') % (productfilter, win, mybuild, version, arch)
        elif os_name:
            info += '\n    - Geselecteerde OS: %s' % os_name

        # Hotfixes
        if hotfixes:
            info += '\n    - Geïnstalleerde hotfixes (%d): %s' % (len(hotfixes), ', '.join(['KB%s' % kb for kb in hotfixes]))
        else:
            info += '\n    - Geïnstalleerde hotfixes: Geen'
        if manual_hotfixes:
            info += '\n    - Manueel gespecificeerde hotfixes (%d): %s' % (len(manual_hotfixes),
        ...
        if not cves:
            print(colored('[+] Definities laden', 'green'))
            cves, date = load_definitions(args.definitions)
            print('    - Aanmaakdatum definities: %s' % date)

        # Determine missing patches
        try:
            print(colored('[+] Missende patches bepalen', 'green'))
        ...
        if args.usekbdate:
            print(colored('[+] Oude kwetsbaarheden uitfilteren', 'green'))
            recentkb = get_most_recent_kb(found)
            if recentkb:
                print('    - Meest recent geïnstalleerde KB is KB%s uitgebracht op %s\n'
                      '    - Alle KBs voor deze datum uitfilteren' % (recentkb['BulletinKB'], recentkb['DatePosted']))
    ...
    if args.hiddenvuln or args.only_exploits or args.impacts or args.severities:
        print(colored('[+] Display filters toevoegen', 'green'))
    ...
    if not args.missingpatches:
        ...
        if args.muc_lookup:
            ...
            print(colored('[!] Vervangende hotfixes zoeken in de Microsoft Update Catalog', 'yellow'))
    ...
    # Display results
    if len(filtered) > 0:
        print(colored('[!] Kwetsbaarheden gevonden!', 'yellow'))
        if args.outputfile:
        ...
        if not args.operating_system and os_names and len(os_names) > 1:
            ...
            print(colored('[i] Extra filters kunnen toevoegd worden met de --os parameter', 'yellow'))

        print(colored('[+] Done. ', 'green') + '%s %s van de %s gevonden kwetsbaarheden.' % (verb, colored(len(filtered), 'yellow'), colored(len(found), 'yellow')))
    else:
        print(colored('[-] Done. Geen kwetsbaarheden gevonden\n', 'green'))
...
def load_definitions(definitions):
    with zipfile.ZipFile(definitions, 'r') as definitionszip:
        ...
        if dbversion > VERSION:
            raise WesException(
                'Het gebruik van definities vereist versie %.2f van wes.py. '
                'Update wes.py met --update-wes.' % dbversion)
...
def determine_product(systeminfo):
    ...
    if len(systeminfo_matches) == 0:
        raise WesException('OS versie kon niet gedetecteerd worden aan de hand van het sysinfo bestand.\n    In het geval dat u een missing.txt script gebruikt, gebruik: wes.py -m missing.txt')
    ...
    if len(win_matches) == 0:
        raise WesException('OS naam kon niet gedecteerd worden aan de hand van het input bestand.')
    win = win_matches[0][1]
    ...
    if len(archs) > 0:
        ...
        logging.warning('Architectuur van het systeem kan niet bepaald worden. Uitgaan van x64...')
        arch = 'x64'
    ...
    # KBs specificly for 2003 R2 (x86/x64) and almost 6000 KBs for 2003 (x86/x64)
        ...
        raise WesException('Vaststellen van Windows versie mislukt, {}'.format(win))
    ...
    if os_name:
        ...
        if os_name.isdigit():
            if int(os_name) >= len(os_names):
                print(colored('[-] Ongeldige OS index gespecificeerd met de --os parameter', 'red'))
                exit(1)
...
def list_operatingsystems(os_names):
    ...
    print(colored('[I] Lijst van operating systems:', 'green'))
    i = 0
...
def print_summary(kbs, sp):
    ...
    print(colored('[-] Missende patches: ', 'red') + '%s' % colored(len(missingpatches), 'yellow'))
    ...
    for line in foundmissing:
        kb = line[0]
        number = line[1]
        print('    - KB%s: patches %s %s' % (kb, number, 'kwetsbaarheid' if number == 1 else 'kwetsbaarheden'))

    # Show in case a service pack is missing
    if sp:
        print(colored('[-] Missende service pack', 'red'))
        print('    - %s' % sp['Title'])
    ...
    if len(missingpatches) > len(grouped):
        ...
        for kb in difference:
            print('    - KB%s: patcht een onbekend aantal kwetsbaarheden.' % kb)
        print(colored('[i] Controleer de details van de onbekende patches op https://support.microsoft.com/help/KBID,\n    bijvoorbeeld https://support.microsoft.com/help/890830 in het geval van KB890830', 'yellow'))
    ...
    message = colored('[i] KB met de meest recente release date', 'yellow')
    print('%s\n'
          '    - ID: KB%s\n'
...
def store_results(outputfile, results):
    print(colored('[+] Resultaten %d schrijven naar %s' % (len(results), outputfile), 'green'))
...
def check_file_exists(value):
    if not os.path.isfile(value):
        raise argparse.ArgumentTypeError('Bestand \'%s\' bestaat niet.' % value)
...
def check_definitions_exists(value):
    if not os.path.isfile(value):
        raise argparse.ArgumentTypeError('Definitions bestand \'%s\' bestaat niet. Probeer %s --update eerst uit te voeren.' % (value, FILENAME))
...
'''.format(FILENAME)
    ...
    # Update definitions
    parser.add_argument('-u', '--update', dest='perform_update', action='store_true', help='Download meest recente lijst van CVEs')
