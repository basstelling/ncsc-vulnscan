...
def xss_format(url):
    ...
    path_vuln_gen = f'Scans/Volledige scans/General-scan--{xss_domain}--.txt'
    path_log_gen = f'Scans/Volledige scans/Logging/Log-General-XSS--{xss_domain}--.txt'
...
try:
    ...
    if resp == '1':
        ...
        if saveData == True:
            with open(r'Scans/Portscan/Port scan '+date_now + ".txt",'w') as f:
                sys.stdout = f
                cls()
        ...
        if saveData == True:
            with open(r'Scans/HTTP headers/Header scan '+date_now + ".txt",'w') as f:
                sys.stdout = f
                cls()
                header_checker(url)
        else:
        ...
        if saveData == True:
            with open(r'Scans/TLS/TLS scan '+date_now + ".txt",'w') as f:
                sys.stdout = f
                cls()
                find_tls(url)
        else:
            ...
            if saveData == True:
                with open(r'Scans/Patch enumeration '+ date_now + ".txt",'w', encoding='utf-8') as f:
                    sys.stdout = f
                    cls()
                    print(output)
            else:
    ...
    #     set_config()
        ...
        if check_url(url) == True:
            ...
            if saveData == True:
                with open(r'Scans/Volledige scans/General-scan--' + stripped_url + '--.txt', 'w') as f:
                    sys.stdout = f
                    f.write(("[i] RESULTATEN VAN DE PORTSCAN:\n"))
                    print(port_scan(ip, '1-100'))
